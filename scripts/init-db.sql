-- Extensions required by OpenSourceCommunity
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";
CREATE EXTENSION IF NOT EXISTS "pgcrypto";

-- Schema (generated from Drizzle migration 0000_mature_zzzax)

CREATE TYPE "public"."crm_provider" AS ENUM('salesforce', 'hubspot');
CREATE TYPE "public"."member_role" AS ENUM('org_admin', 'moderator', 'member', 'guest');
CREATE TYPE "public"."plan" AS ENUM('starter', 'growth', 'enterprise');
CREATE TYPE "public"."sso_provider" AS ENUM('saml', 'oidc');
CREATE TYPE "public"."forum_category_visibility" AS ENUM('public', 'members', 'restricted');
CREATE TYPE "public"."forum_report_resource_type" AS ENUM('thread', 'post');
CREATE TYPE "public"."forum_report_status" AS ENUM('pending', 'reviewed', 'dismissed');
CREATE TYPE "public"."forum_target_type" AS ENUM('thread', 'category');
CREATE TYPE "public"."forum_thread_status" AS ENUM('open', 'resolved', 'locked', 'archived');
CREATE TYPE "public"."idea_status" AS ENUM('new', 'under_review', 'planned', 'in_progress', 'shipped', 'declined');
...
CREATE TYPE "public"."kb_visibility" AS ENUM('public', 'members', 'restricted');
CREATE TYPE "public"."si_alert_type" AS ENUM('volume_spike', 'crisis', 'competitor_mention');
CREATE TYPE "public"."si_keyword_group_type" AS ENUM('brand', 'competitor', 'custom');
CREATE TYPE "public"."si_mention_status" AS ENUM('new', 'reviewed', 'actioned');
CREATE TYPE "public"."si_sentiment" AS ENUM('positive', 'negative', 'neutral', 'mixed');
...
CREATE UNIQUE INDEX IF NOT EXISTS "members_tenant_user_unique" ON "members" USING btree ("tenant_id","user_id");
CREATE UNIQUE INDEX IF NOT EXISTS "forum_categories_tenant_slug_unique" ON "forum_categories" USING btree ("tenant_id","slug");
CREATE UNIQUE INDEX IF NOT EXISTS "forum_follows_tenant_member_target_unique" ON "forum_follows" USING btree ("tenant_id","member_id","target_type","target_id");
CREATE UNIQUE INDEX IF NOT EXISTS "forum_reactions_tenant_post_member_emoji_unique" ON "forum_reactions" USING btree ("tenant_id","post_id","member_id","emoji");
CREATE UNIQUE INDEX IF NOT EXISTS "idea_votes_tenant_idea_member_unique" ON "idea_votes" USING btree ("tenant_id","idea_id","member_id");
CREATE UNIQUE INDEX IF NOT EXISTS "event_rsvps_tenant_event_member_unique" ON "event_rsvps" USING btree ("tenant_id","event_id","member_id");
CREATE UNIQUE INDEX IF NOT EXISTS "course_enrollments_tenant_course_member_unique" ON "course_enrollments" USING btree ("tenant_id","course_id","member_id");
CREATE UNIQUE INDEX IF NOT EXISTS "webinar_registrations_tenant_webinar_member_unique" ON "webinar_registrations" USING btree ("tenant_id","webinar_id","member_id");
CREATE UNIQUE INDEX IF NOT EXISTS "kb_categories_tenant_slug_unique" ON "kb_categories" USING btree ("tenant_id","slug");
